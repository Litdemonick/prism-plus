// ─── Piezas compartidas por los servidores de hentaila ───────────────────────
//
...
// hentaila — ninguna otra extensión las toca.
...
export async function pedir(
  ...
  try {
    ...
    console.log(`[ht] no se pudo pedir ${url.slice(0, 45)} :: ${(e as Error)?.message ?? e}`);
    return null;
  }
}
...
//
// Lo necesitan VidHide y StreamWish: la dirección no está en el HTML tal cual,
 ...
 * sitios la guardan. Es lo único que comparten VidHide y StreamWish: cada uno
 * tiene su propia carpeta y su propio resolver, así que arreglar uno no toca al
 * otro.
 */
