// ─── Piezas compartidas por los servidores de tioanime ───────────────────────
//
...
// tioanime — ninguna otra extensión las toca.
...
export async function pedir(
  ...
  try {
    ...
    console.log(`[ta] no se pudo pedir ${url.slice(0, 45)} :: ${(e as Error)?.message ?? e}`);
    return null;
  }
}
...
//
// Lo usa el camino de respaldo de `index.ts`: la dirección no siempre está en el
