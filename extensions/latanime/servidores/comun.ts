// ─── Piezas compartidas por los servidores de latanime ───────────────────────
//
...
// latanime — ninguna otra extensión las toca.
...
export async function pedir(
  ...
  try {
    ...
    console.log(`[la] no se pudo pedir ${url.slice(0, 45)} :: ${(e as Error)?.message ?? e}`);
    return null;
  }
}
 ...
 *
 * Lo piden los de la familia XFileSharing —acá hexload y savefiles—, que no
 * traen el vídeo en la página: lo van a buscar por AJAX. `pedir` solo hace GET.
 */
export async function postForm(
  ...
  try {
    ...
    console.log(`[la] POST falló ${url.slice(0, 45)} :: ${(e as Error)?.message ?? e}`);
    return null;
  }
}
...
//
// Lo necesita mixdrop: la dirección no está en el HTML tal cual,
 ...
 * sitios la guardan. Acá la usa el camino de respaldo de `index.ts`, para un
 * servidor que el sitio sume y todavía no tenga carpeta propia.
 */
