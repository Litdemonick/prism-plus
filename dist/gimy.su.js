...
export default class extends Extension {
  ...
  async search(kw, page) {
    ...
    const res = await this.req(`/${kw}----------${page}---.html`, searchbase);
  ...
  async detail(url) {
    ...
    const episodes = [];
    if (isSu) {
      const mainRegion2 = await this.querySelectorAll(res, "div.col-xs-12.col-sm-12.col-md-9");
      const mainRegionelement2 = mainRegion2[0].content;
      const title2 = await this.getAttributeText(mainRegionelement2, "a.video-pic", "title");
      const bsxList2 = await this.querySelectorAll(res, "div.clearfix.layout-box.playlist.playlist-mobile");
      const patrial_cover_url2 = mainRegionelement2.match(/url\((.+?)\)/)[1];
      const cover2 = patrial_cover_url2.match(/https/) ? patrial_cover_url2 : "https://gimy.su" + patrial_cover_url2;
      for (const element of bsxList2) {
        ...
        for (const e of selectList) {
          ...
          const url2 = await this.getAttributeText(el, "a", "href");
          urls.push({
            name,
            url: url2
          });
  ...
  async watch(url) {
    ...
    if (!isSu) {
      ...
      const res2 = await this.req(url, baseUrl);
      const vidInfo = res2.match(/player_data=(.+?)</)[1].replace(/\//g, "/");
      console.log(vidInfo);
