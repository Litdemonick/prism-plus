...
export default class extends Extension {
  ...
  async latestEvents(page) {
    const res = await this.request("/public/events/recent?page=" + (page != null ? page : 1));
  ...
  async searchEvent(kw, page, filter) {
    ...
    const res = await this.request(
      "/public/events/search?page=" + (page != null ? page : 1) + "&q=" + encodeURIComponent(kw)
    );
    return res.events.map((event) => ({
      title: event.title,
      url: event.url,
  ...
  async detailConference(url) {
    ...
    let urlsSd = urls.map(function(url2) {
      return {
        name: url2.name,
        url: url2.url + "#sd"
      };
    });
    let urlsMp3 = urls.map(function(url2) {
      return {
        name: url2.name,
        url: url2.url + "#mp3"
      };
    });
    let urlsOpus = urls.map(function(url2) {
      return {
        name: url2.name,
        url: url2.url + "#opus"
      };
  ...
  async watch(url) {
    ...
    let parts = url.split("#");
    url = parts[0];
    let mode = (_a = parts[1]) != null ? _a : "fullhd";
    const event = await this.request(url);
    let recordings = event.recordings.filter(function(recording) {
      if (mode === "mp3") {
  ...
  removeHost(url) {
    url = url.replace("https://api.media.ccc.de", "");
    url = url.replace("https://media.ccc.de", "");
    return url;
  }
}
