...
export default class extends Extension {
  ...
  async $get(params, count = 2, timeout = 4e3) {
    try {
      const list = this.domains.map(
        (e) => this.request("/api.php/provide/vod?ac=detail" + params, {
  ...
  async search(kw, page, filter) {
    ...
    if (!((_a = filter == null ? void 0 : filter.genres) == null ? void 0 : _a[0])) {
      return this.latest(page);
    }
    const res = await this.$get(`&t=${(_c = (_b = filter == null ? void 0 : filter.genres) == null ? void 0 : _b[0]) != null ? _c : ""}&pg=${page}`);
    return res.list.map((e) => ({
      title: e.vod_name,
      url: `${e.vod_id}`,
      cover: e.vod_pic,
