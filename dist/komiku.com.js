...
export default class extends Extension {
  ...
  async get_filter() {
    ...
    if (!this.checkCache("@status")) {
      ...
      __privateGet(this, _cache).set("@status", status);
    }
    if (!this.checkCache("@types")) {
      ...
      __privateGet(this, _cache).set("@types", types);
    }
    if (!this.checkCache("@orders")) {
      ...
      __privateGet(this, _cache).set("@orders", orders);
    }
  }
  async createFilter() {
    return {
      "type": {
        ...
        min: 0,
        default: "type=",
        options: __privateGet(this, _cache).get("@types")
      },
      "order": {
        title: "Orders",
        max: 1,
        ...
        options: __privateGet(this, _cache).get("@orders")
      },
      "status": {
        title: "Status",
        max: 1,
        min: 0,
        default: "status=",
        options: __privateGet(this, _cache).get("@status")
      },
      "genre": {
        title: "Genres",
        max: 1,
  ...
  async search(kw, page, filter) {
    const filter_genre = (filter == null ? void 0 : filter.genre) && filter.genre.length > 0 ? filter.genre[0] : "";
    const filter_type = (filter == null ? void 0 : filter.type) && filter.type.length > 0 ? filter.type[0] : "";
    const filter_order = (filter == null ? void 0 : filter.order) && filter.order.length > 0 ? filter.order[0] : "";
    const filter_status = (filter == null ? void 0 : filter.status) && filter.status.length > 0 ? filter.status[0] : "";
    let arr_query = [];
    if (kw) {
      arr_query.push(`s=${kw}`);
    }
    ...
    let list_items = [];
    for (const element of items) {
      const url2 = await this.getAttributeText(element.content, "a", "href");
