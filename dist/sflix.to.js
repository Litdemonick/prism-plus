...
export default class extends Extension {
  ...
  async detail(url) {
    ...
    if (url.includes("movie")) {
      const server_res2 = await this.request(`/ajax/episode/list/${video_id}`);
      const server = server_res2.match(/data-id="\d+"/g).map((element) => {
        return element.match(/\d+/)[0];
      });
      const video_server = server_res2.match(/>\w+</g).map((element) => {
  ...
  async watch(url) {
    ...
    const server_res = await this.request(`/ajax/episode/servers/${url}`);
