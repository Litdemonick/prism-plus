...
export default class extends Extension {
  ...
  async latest(page) {
    ...
    const latest = res.match(new RegExp('<div class="page-item-detail manga">(.*?)<\\/div>', "gs"));
  ...
  async search(kw, page) {
    ...
    const resizeValue = await this.getSetting("imageResize");
    const regex = new RegExp('<div class="row c-tabs-item__content">(.*?)<\\/div>', "gs");
  ...
  async detail(url) {
    ...
    const chaptersRegex = new RegExp(`<li\\s+class\\s*=\\s*["']\\s*wp-manga-chapter\\s*["']\\s*>(.*?)<\\/li>`, "gs");
  ...
  async watch(url) {
    ...
    const images = res.match(new RegExp(`<div\\s+class\\s*=\\s*["']\\s*page-break\\s+no-gaps\\s*["']\\s*>(.*?)<\\/div>`, "gs")).map((element) => element.match(/data-src="(.+?)"/)).filter((imgMatch) => imgMatch).map((imgMatch) => imgMatch[1].trim());
    return {
      urls: images || []
    };
  }
