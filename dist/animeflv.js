...
// @description  Anime con subtítulos y doblaje en español desde AnimeFLV
// ==/PrismHubExtension==
...
${u}`;
  ...
  async function watch(url) {
    ...
    const all = [...(_a = videos["LAT"]) != null ? _a : [], ...(_b = videos["SUB"]) != null ? _b : []].map((v) => {
      var _a2, _b2;
      return { server: v.server || v.title || "", embed: (_b2 = (_a2 = v.code) != null ? _a2 : v.url) != null ? _b2 : "" };
...
export default class extends Extension {
  async latest(page) { return io_prismhub_animeflv.latest(page); }
  async search(kw, page, filter) { return io_prismhub_animeflv.search(kw, page, filter); }
  async createFilter(filter) { return typeof io_prismhub_animeflv.createFilter === 'function' ? io_prismhub_animeflv.createFilter(filter) : {}; }
  async detail(url) { return io_prismhub_animeflv.detail(url); }
  async checkUpdate(url) { return typeof io_prismhub_animeflv.checkUpdate === 'function' ? io_prismhub_animeflv.checkUpdate(url) : {}; }
  ...
  async watch(url) {
    ...
    var r = await io_prismhub_animeflv.watch(url);
