...
export default class extends Extension {
  ...
  async detail(url) {
    ...
    const desc = (_a = res.match(/<div id="vod_content" class="hidden xl:inline-block">([\s\S]+?)<\/div>/)[1]) == null ? void 0 : _a.replace("<!-- \u5F71\u7247\u7B80\u4ECB -->", "").trim();
    ...
    for (let i = 0; i < episodeGroupNames.length; i++) {
      ...
      const groupEpisodesList = res.match(
        new RegExp(
          `<div data-sid="${i + 1}" class="video_list play_list_asc hidden fn-clear grid grid-cols-4 gap-3 md:grid-cols-6 md:gap-4 xl:grid-cols-8 xl:gap-4">([\\s\\S]+?)<\\/div>`
        )
      )[1];
