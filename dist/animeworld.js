...
export default class extends Extension {
  ...
  async requestWSetting(url2, raw = false) {
    if (raw == false) {
  ...
  async latest(page) {
    let url2 = page > 1 ? `/updated?page=${page}` : "/updated";
    const page_domain = await this.getSetting("domain_animeworld");
    let list_item = [];
    const fetch_anime_latest = await this.requestWSetting(url2);
    ...
    for (const element of element_latest) {
      ...
      const url3 = await this.getAttributeText(
        element.content,
        "a.name",
        "href"
      );
  ...
  async detail(url2) {
    const page_domain = await this.getSetting("domain_animeworld");
    const fetch_detail_anime = await this.requestWSetting(url2);
    const mirror_available = ["AnimeWorld Server", "Streamtape", "FileMoon"];
    ...
    for (const mirror of Object.keys(streamable)) {
      ...
      for (const element of element_episode) {
        ...
        mirror_episode.push({ name, url: url3 });
      }
      episodes.push({
        title: mirror,
        urls: mirror_episode
  ...
  async watch(url2) {
    ...
    const fetch_link_video = await this.request("", {
      headers: {
        "Miru-Url": `${page_domain}/api/episode/info?id=${url2}&alt=0`
      }
    });
  ...
  async mirror_filemoon(url) {
    ...
    const match = fetch_url.match(new RegExp("eval(.+?)<\\/script>", "gs"));
    if (!match) return "";
  ...
  cariMatch(input, regex) {
    ...
    return match2 ? match2.length > 2 ? match2 : match2[1] : null;
  }
}
