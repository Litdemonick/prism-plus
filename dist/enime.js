...
var io_prismhub_enime = (() => {
  ...
  // extensions/enime/index.ts
  var API = "https://api.enime.moe";
  function enimeTitle(t) {
    return t.english ?? t.romaji ?? t.native;
  }
  async function latest(page) {
    const data = await getJson(`${API}/recent?page=${page}`);
  ...
  async function detail(url) {
    const data = await getJson(`${API}/anime/${url}`);
    return {
  ...
  async function watch(url) {
    const data = await getJson(`${API}/episode/${url}`);
...
;(function(){
  ...
  var _m=typeof io_prismhub_enime!=='undefined'?io_prismhub_enime:null;
