...
export default class extends Extension {
  ...
  async detail(url) {
    ...
    const pattern = new RegExp('<div id="noidungm".*?>(.*?)<\\/div>', "s");
    const match = res.match(pattern);
    const desc = match ? match[1].replace(/<[^>]+>/g, "").trim().replace(/&#39;/g, "'").replace(/&nbsp;/g, " ").replace(/’/g, "'").replace(/&ldquo;/g, '"').replace(/&rdquo;/g, '"') : "";
    const episodes = [];
    const epiList = res.match(/<div class="row">([\s\S]+?)<\/div>/g);
    epiList.forEach((element) => {
      const name = element.match(/<a.*>(.+?)<\/a>/)[1];
      const url2 = element.match(/href="([^"]+)"/)[1];
      episodes.push({
        name,
        url: url2
      });
  ...
  async watch(url) {
    ...
    chapterContentDiv = chapterContentDiv.replace(/<[^>]+>/g, "\n").replace(/\<p\>/g, "").replace(/&#39;/g, "'").replace(/&nbsp;/g, " ").replace(/’/g, "'").replace(/&ldquo;/g, '"').replace(/&rdquo;/g, '"');
