...
function sha1_raw(bytes) {
  ...
  p.push(0, 0, 0, 0);
  p.push(ml >>> 24 & 255, ml >>> 16 & 255, ml >>> 8 & 255, ml & 255);
  var w = new Array(80);
...
function hmacSha1Hex(key, msg) {
  var kb = [];
  for (var i = 0; i < key.length; i++) kb.push(key.charCodeAt(i) & 255);
  if (kb.length > 64) kb = sha1_raw(kb);
  while (kb.length < 64) kb.push(0);
  var ipad = kb.map(function(b) {
    return b ^ 54;
  });
  var opad = kb.map(function(b) {
    return b ^ 92;
  });
  var mb = [];
  for (var i = 0; i < msg.length; i++) mb.push(msg.charCodeAt(i) & 255);
    ...
    if (surl) {
      try {
        ...
        if (infoData && infoData.errno === 0 && Array.isArray(infoData.list) && infoData.list.length > 0) {
          ...
          const streamUrl = "https://www.1024terabox.com/share/streaming?uk=" + uk + "&shareid=" + shareid + "&type=M3U8_FLV_264_360&fid=" + fsId + "&sign=" + sign + "&timestamp=" + ts + "&esl=1&isplayer=1&ehps=1&clienttype=0&app_id=250528&web=1&channel=dubox";
