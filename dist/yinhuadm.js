...
export default class extends Extension {
  async latest(page) {
    ...
    const res2 = await this.request(keyword, hh);
    ...
    for (let i2 = 0; i2 < ul.length; i2++) {
      let title = await this.querySelector(ul[i2].content, "strong").text;
      let cover = await this.querySelector(
        ul[i2].content,
        "img.lazy.lazyload"
      ).getAttributeText("data-original");
      let url2 = await this.querySelector(ul[i2].content, "a").getAttributeText(
        "href"
      );
      console.log({
        title,
  ...
  async detail(url2) {
    console.log(url2);
    let res2 = await this.request(url2);
    ...
    let ul = await this.queryXPath(res2, '//a[@class="module-play-list-link"]').allHTML;
    let chapter = [];
    for (let i2 = 0; i2 < ul.length; i2++) {
      chapter.push({
        name: await this.queryXPath(ul[i2], "//span").text,
        url: await this.queryXPath(ul[i2], "//a/@href").attr
      });
    }
    console.log({
      title,
  ...
  async watch(url) {
    ...
    let viewport = ids[1].match(/id="now_(.+)"/)[1], charset = ids[0].match(/id="now_(.+)"/)[1], list = [], list_sort = [], str = "";
